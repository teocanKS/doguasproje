...
const getDetailedHistory = async (req, res) => {
    ...
    `;
        ...
        let finalQuery;
        if (type === 'Satış') {
            finalQuery = `${musteriQuery} ORDER BY tarih DESC LIMIT $${paramIndex} OFFSET $${paramIndex + 1}`;
        } else if (type === 'Alış') {
            finalQuery = `${tedarikciQuery} ORDER BY tarih DESC LIMIT $${paramIndex} OFFSET $${paramIndex + 1}`;
        } else {
            finalQuery = `(${musteriQuery}) UNION ALL (${tedarikciQuery}) ORDER BY tarih DESC LIMIT $${paramIndex} OFFSET $${paramIndex + 1}`;
        }
